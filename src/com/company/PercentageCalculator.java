...
public class PercentageCalculator {
    ...
    List<Transaction> viewedTransactions = new ArrayList<Transaction>();
    ...
    public void calculateOccurrence() {
        ...
        for (Transaction transaction : transactions) {
            if (!allreadyLogged(viewedTransactions,transaction)) {
                occurrence.put(transaction.getOperation(), occurrence.get(transaction.getOperation()) + 1);
                viewedTransactions.add(transaction);
            }
        }
    }
    ...
    public void calculatePercentage() {
        ...
        for (Transaction transaction : viewedTransactions) {
            final Double unformatted = occurrence.get(transaction.getOperation()) / viewedTransactions.size() * 100;
